...
## ✅ Concluído (recente)
- **Disparos — prospecção por lista morna (import + etiqueta + lotes de 30).** Fluxo: (1) importar CSV de contatos **já etiquetando em lote** (campo de etiqueta na tela de Contatos → aplicada a todos os importados via `array_append`); modelo de planilha para baixar em `public/modelo-contatos.csv`. (2) Em Disparos, filtrar pela etiqueta e enviar **de 30 em 30 sem repetir**: novo `Contact.broadcast_sent_at` marca quem já recebeu o convite; cada clique pega o **próximo lote** de até 30 elegíveis (exclui `opted_out` e já-convidados) e a resposta traz `remaining` para o progresso ("Restam N — clique de novo"). Seletor de etiquetas agora inclui as tags realmente usadas nos contatos (união com `lead_tags`). **Guarda-corpo:** aviso destacado de que a lista deve ser de contatos com relação/consentimento — lista fria arrisca **ban do número** (Meta) e LGPD; para atrair novos, usar CTWA/Google Ads (cliente inicia). Mantém o formato consentimento SIM/SAIR e o template aprovado.
- **Webhook do WhatsApp — confiabilidade/latência (crítico p/ escala).** Causa dos "bot demorou / às vezes nem respondeu": o webhook **esperava a IA responder antes de devolver 200 ao Meta**. Como a IA pode levar segundos (loop de tools + latência), o Meta estourava o timeout do webhook e **reenviava o evento** → duplicatas, `message.id` repetido batendo no unique → 500 → mais retentativas → acúmulo (pior de madrugada com cold start + latência do modelo). Correções: (1) **ACK imediato** — responde 200 na hora e processa em **segundo plano** (processo Railway é persistente); (2) **deduplicação** por `message.id` (ignora reenvios, sem resposta dupla); (3) **timeout (60s) + maxRetries (2)** nos clientes Anthropic; (4) **fallback** — se a IA falha/expira, manda um aviso curto em vez de silêncio; (5) o POST nunca lança (sempre 200). Não era problema de volume (4 tenants), e sim de arquitetura do ACK.
- **Promaster — catálogo à prova de provisionamento incompleto.** Causa raiz: o provisionamento do schema do tenant roda `npx prisma db push` em runtime (activate/provision) e pode falhar silenciosamente (a rota `activate` guarda o erro em `steps.schemaError` mas retorna `ok:true`), deixando a tabela `products` ausente. Sintoma: "Sincronizado: 0 produtos" (falso sucesso) porque todo upsert caía num catch silencioso. Correções: (1) `syncTenantProducts` agora chama `ensureProductsTable(schema)` — DDL idempotente (`CREATE TABLE/INDEX IF NOT EXISTS`, sem depender do CLI do Prisma) que espelha o model `Product`, então o 1º sync (manual ou cron horário) **cria a tabela sozinho**; (2) se o feed tem itens mas nada é gravado, retorna `ok:false` com a causa real em vez de fingir "0 produtos". Vale para todo tenant Promaster novo e antigo.
